...
MyGame = ig.Box2DGame.extend({
	...
	getBodyUnderMouse: function(){
        ...
        function GetBodyCallBack(fixture){
                ...
                console.log(fixture.GetUserData().name + " grabbed and stored, id is " + fixture.GetUserData().id );
	...
	destroyMouseJoint: function(){
		if(this.mouseOverBody != false){
            ...
            if( 
            	...
            	ig.game.mouseOverClass.name == "UPPERLEG" ||
            	ig.game.mouseOverClass.name == "LOWERLEG" 
            ){
            	//freeze body
            	ig.game.mouseOverBody.SetType( Box2D.Dynamics.b2Body.b2_staticBody );
            	//store it in frozenBodies Array unless it was the last one clicked
            	//if the array has no length, this is the first one
            	if( !ig.game.frozenBodies.length ){ 
            		console.log('first body frozen & stored');
            		ig.game.frozenBodies.push( ig.game.mouseOverClass ); 
            		console.log(ig.game.frozenBodies);
            	}
            	else {
            		console.log(" frozen body length -1 id: "+ ig.game.frozenBodies[ ig.game.frozenBodies.length - 1 ].id + " mouseOverBody id: " + ig.game.mouseOverClass.id );
            		//loop through frozenbodies, check if it's stored already
            		for( var i =  0 ; i < ig.game.frozenBodies.length ; i++ ){
		        		if(ig.game.frozenBodies[ i ].id == ig.game.mouseOverClass.id ){
		        			//there is a match
		        			ig.game.bodiesMatch = true;
		        		}

		        	}
		        	//was there a match?
		        	if( ig.game.bodiesMatch == false ){
		        		console.log('body is novel, store it');
	        			//no match, store it
	        			ig.game.frozenBodies.push( ig.game.mouseOverClass );
	        			console.log(ig.game.frozenBodies);
		        	}
		        	else{ 
		        		//there was a match, switch it back
		        		console.log('match found, do not store body');
		        		ig.game.bodiesMatch = false;
		        	}
        		...
        		if( ig.game.frozenBodies.length > 3 ){
        			console.log( 'length is 3, unfreeze oldest and splice ');
        			ig.game.frozenBodies[0].body.SetType( Box2D.Dynamics.b2Body.b2_dynamicBody );
